...
pub fn recognize_text(base64_image: &str) -> Result<RecognizeTextResponse, OcrError> {
    unsafe {
        ...
        let ns_image = ns_image
            .ok_or_else(|| OcrError::InvalidRequest("failed to create NSImage from data".into()))?;
...
async fn wait_for_translation_response(
    ...
    tokio::task::spawn_blocking(
        move || match receiver.recv_timeout(SYSTEM_TRANSLATION_TIMEOUT) {
            Ok(result) => result,
...
pub async fn translate(request: &TranslateRequest) -> Result<TranslateResponse, TranslationError> {
    ensure_observer();
    ...
    let target = request
        .target_language
        .as_deref()
        .ok_or_else(|| TranslationError::InvalidRequest("target_language is required".into()))?;
...
pub async fn look_up(request: &LookUpRequest) -> Result<LookUpResponse, DictionaryError> {
    ...
    unsafe {
        let text =
            CFStringCreateWithCString(std::ptr::null(), c_word.as_ptr(), CF_STRING_ENCODING_UTF8);
        if text.is_null() {
...
fn extract_pronunciation(text: &str) -> Vec<WordPronunciation> {
    ...
    if has_label {
        ...
        let flush_current = |result: &mut Vec<WordPronunciation>,
