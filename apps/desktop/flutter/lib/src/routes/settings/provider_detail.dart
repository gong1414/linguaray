...
class _ProviderDetailPageState extends State<ProviderDetailPage> {
  ...
  late Map<String, String> _persistedFields;
  ...
  void initState() {
    super.initState();
    _persistedFields = Map<String, String>.of(widget.provider.fields);
    _fieldControllers = {
      for (final key in _fieldKeys)
        key: TextEditingController(
          text: isSecretField(key) ? '' : widget.provider.fields[key] ?? '',
        ),
    };
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      for (final entry in _fieldControllers.entries) {
        if (isSecretField(entry.key)) entry.value.clear();
      }
      await Future.wait([
  ...
  Future<void> _delete() async {
    ...
    try {
      ...
      providerCredentialsController.deleteProvider(widget.provider.id);
      await Future.wait([
        settingsStore.reloadProviders(),
        settingsStore.reloadServices(),
      ]);
