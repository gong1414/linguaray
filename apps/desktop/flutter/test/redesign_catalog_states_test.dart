...
void main() {
  for (final platform in [TargetPlatform.macOS, TargetPlatform.windows]) {
    for (final size in [const Size(1000, 700), const Size(780, 520)]) {
      for (final brightness in Brightness.values) {
        testWidgets(
          'all settings share their frame at $size / $brightness / $platform',
          (tester) async {
            tester.view
              ..devicePixelRatio = 1
              ..physicalSize = size;
            ...
            for (final section in SettingsSection.values) {
              await tester.pumpWidget(
                MaterialApp(
                  theme: LinguaRayMaterialTheme.forBrightness(
                    brightness,
                    platform: platform,
                  ),
              ...
              expect(
                position,
                origin,
                reason: 'Title position: ${section.name}',
              );
              final material = tester.widget<Material>(
                find
                    .descendant(of: page, matching: find.byType(Material))
                    .first,
              );
              expect(
                material.color,
                LinguaRayMaterialTheme.forBrightness(
                  brightness,
                  platform: platform,
