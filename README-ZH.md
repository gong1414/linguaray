...
| 服务商配置与凭据安全存储 | 已实现 |
| 历史记录、收藏、术语库与生词本 | 已实现 |
| 词典查询与文字朗读 | 内置离线 ECDICT；受支持的 macOS 版本可使用 Apple 系统翻译与系统朗读 |
| 输入行为与常用语言排序 | 已实现 |
| 本地 API、URL Scheme、PopClip、SnipDo 与 Raycast 集成 | 已实现 |
| 本地备份恢复、代理模式与带校验的更新检查 | 已实现 |
| macOS 与 Windows 桌面构建 | 已由 CI 构建和验证 |
| 签名安装包与稳定版本 | 签名草稿发布流程已就绪；稳定版本尚未发布 |
...
发布维护者还应阅读[发布指南](docs/RELEASING.md)。
...
内置数据与依赖的版权说明见[第三方声明](THIRD_PARTY_NOTICES.md)。
