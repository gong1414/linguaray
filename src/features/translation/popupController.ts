...
import { openSettingsWindow, translateSelection, ttsSpeak, ttsStop } from "./popup-ipc";
import { ensureProviderNameMap, engineLabel as sharedEngineLabel } from "./providerNames";
...
export function usePopupController() {
  ...
  // Bumped when the shared provider-name map finishes loading so card labels
  // re-render with resolved names (the map itself lives in providerNames).
  ...
  useEffect(() => {
    ...
    // Provider name map LAST (best-effort; labels fall back below). Shared
    // module-level cache — same load-after-listeners ordering as before.
    void ensureProviderNameMap().then(() => {
      if (ref.current.cancelled) return;
      setNameTick((n) => n + 1);
    });
  ...
  const engineLabel = useCallback((raw: string): string => sharedEngineLabel(raw), [nameTick]);
  ...
  return {
    ...
    speak: (text: string) => ttsSpeak(text),
    stopSpeaking: () => ttsStop(),
    favoriteText: (source: string, text: string) => addVocabulary(source || text, text, "zh"),
    openSettings: (section: string) => openSettingsWindow(section),
  };
}
